...
class CartProvider with ChangeNotifier {
  ...
  Future<void> syncWithBackend() async {
    ...
    try {
      ...
      // Fusionar con local (local tiene prioridad)
  ...
  List<CartItem> _mergeCartItems(
    ...
    // Primero agregar items locales
    for (final item in local) {
      merged[item.product.id] = item;
    }
    
    // Luego agregar items remotos que no existan en local
  ...
  void _addWithLock(Product product) {
    Timer.run(() {
